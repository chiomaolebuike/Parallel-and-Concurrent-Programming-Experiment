 ...
 * Parallel version using RecursiveTask for Fork-Join framework.
 *
 * C. Olebuike 2025
 */
...
public class HuntParallel extends RecursiveTask<Integer> {
	...
	/**
     * Compute method for RecursiveTask - finds the local maximum mana from an initial starting point
     * 
	...
	public int findManaPeak() {
		...
		while(!dungeon.visited(posRow, posCol)) { // stop when hit existing path
			...
			switch(next) {
				case STAY: return power; //found local maximum
				case LEFT:
					posRow--;
					break;
				case RIGHT:
